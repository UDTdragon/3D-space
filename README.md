...
상세 화면은 전체 이미지 영역과 하단 내비게이션으로 구성됩니다. 통 이미지는 약 `1210 × 691px` 또는 같은 비율로 제작하면 이미지 영역을 빈 공간 없이 채우기 좋습니다. 이미지 안의 글자와 중요한 요소는 가장자리에서 충분히 안쪽에 배치하고, `public/artworks/` 파일을 교체하면 됩니다. 하단의 `01 / 06`, 이전·다음 버튼과 선택적인 PDF 버튼은 웹에서 별도로 표시됩니다.
