...
export async function createViewer(container,{onArtwork,onStation,onBusy,onError}){
 ...
 stations.forEach((station,index)=>{if(station.id==='discovery')return;const ring=new T.Mesh(new T.RingGeometry(.20,.25,64),new T.MeshBasicMaterial({color:'#ffffff',side:T.DoubleSide,transparent:true,opacity:.95,depthTest:true}));ring.rotation.x=-Math.PI/2;ring.position.set(station.id==='connection'?station.position[0]-.3:station.position[0],.021,station.id==='connection'?station.position[2]+1:station.position[2]);ring.userData.station=index;markers.add(ring);
