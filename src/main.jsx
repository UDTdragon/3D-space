...
function App(){const host=useRef(),viewer=useRef();const [entered,setEntered]=useState(false),[entering,setEntering]=useState(false),[ready,setReady]=useState(false),[busy,setBusy]=useState(true),[station,setStation]=useState(0),[art,setArt]=useState(null),[error,setError]=useState(''),[retry,setRetry]=useState(0),[list,setList]=useState(false);
 ...
 return <main className={entered?'exhibition entered':'exhibition'}>
  ...
  <Dialog open={!!art} onOpenChange={v=>{if(!v)setArt(null)}}><DialogContent className="art-dialog" showCloseButton={false}>{art&&<><DialogTitle className="sr-only">{art.title}</DialogTitle><DialogDescription className="sr-only">{art.description}</DialogDescription><button className="close-dialog" aria-label="닫기" onClick={()=>setArt(null)}><X/></button><div className="art-image-stage"><img className="large-art" src={asset(art.image)} alt={art.title}/></div><div className="art-pagination"><span>0{artworks.findIndex(a=>a.id===art.id)+1} / 0{artworks.length}</span><div>{art.pdf&&<a href={asset(art.pdf)} target="_blank" rel="noopener noreferrer" className="pdf-link"><FileText size={17}/> PDF 자료 보기</a>}<button aria-label="이전 작품" onClick={()=>setArt(artworks[(artworks.findIndex(a=>a.id===art.id)+5)%6])}><ArrowLeft size={18}/></button><button aria-label="다음 작품" onClick={()=>setArt(artworks[(artworks.findIndex(a=>a.id===art.id)+1)%6])}><ArrowRight size={18}/></button></div></div></>}</DialogContent></Dialog>
