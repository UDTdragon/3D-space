...
try{const page=await browser.newPage({viewport:{width:1920,height:1080}});page.on('pageerror',e=>console.error(e));await page.goto('http://127.0.0.1:5173/bake.html?size='+(process.env.PANORAMA_SIZE||2048));await page.waitForFunction(()=>window.bakeReady,{},{timeout:90000});
 const write=async(path,data)=>fs.writeFile(path,Buffer.from(data.split(',')[1],'base64'));
 await fs.mkdir('public/assets',{recursive:true});await write('public/assets/representative.webp',await page.evaluate(()=>window.renderPreview()));await write('public/assets/entrance-exterior.webp',await page.evaluate(()=>window.renderExterior()));console.log('Interior and exterior views saved.');
