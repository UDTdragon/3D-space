...
test('outside entry, clear interior, event one and floor-only navigation',async({page})=>{
 ...
 for(let next=1;next<4;next++){await expect(page.locator('.loading')).toHaveCount(0);await expect(page.locator('.room')).not.toHaveClass(/changing/);const p=screen([stations[next].position[0],.021,stations[next].position[2]],next-1,-Math.PI/2+22*.07,.025-4*.06);await page.mouse.click(p.x,p.y);await expect(page.locator('.room')).toHaveAttribute('data-station',String(next),{timeout:15000});}
